...
class LicenseController
{
    ...
    public function verify(Request $request)
    {
        try {
            ...
            if ($license->expires_at && $license->plan->grace_period_days > 0) {
                $gracePeriodUntil = $license->expires_at->addDays((int) $license->plan->grace_period_days)->toIso8601String();
            }
    ...
    public function ping(Request $request)
    {
        try {
            ...
            if (in_array($license->status, ['revoked', 'suspended'])) {
                $response = ['status' => $license->status, 'message' => "License has been {$license->status}."];
                ApiAuditLogger::log($request, '/licenses/ping', 422, $license->status, "License {$license->status}.", $response);
            ...
            if ($license->expires_at && now()->isAfter($license->expires_at)) {
                $license->update(['status' => 'expired']);
                $response = ['status' => 'expired', 'message' => 'License has expired.'];
                ApiAuditLogger::log($request, '/licenses/ping', 422, 'expired', 'License expired.', $response);
            ...
            if (! $activation || $activation->status !== 'active') {
                $response = ['status' => 'invalid', 'message' => 'Domain not activated or revoked.'];
                ApiAuditLogger::log($request, '/licenses/ping', 422, 'invalid', 'Domain not activated or status not active.', $response);
