...
class PaymentController extends Controller
{
    ...
    public function createSnapToken(Request $request)
    {
        ...
        if ($request->promo_code) {
            ...
            if ($promo && $promo->isValid()) {
                ...
                if ($promo->product_id && $plan->product_id !== $promo->product_id) $isEligible = false;
                if ($promo->license_plan_id && $plan->id !== $promo->license_plan_id) $isEligible = false;
                if ($promo->min_order_amount && $originalAmount < $promo->min_order_amount) $isEligible = false;
    ...
    private function handleFulfillment(Order $order)
    {
        $license = DB::transaction(function () use ($order) {
            ...
            if (License::where('order_id', $order->id)->exists()) return null;
