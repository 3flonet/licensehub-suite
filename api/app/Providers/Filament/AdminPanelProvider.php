...
class AdminPanelProvider extends PanelProvider
{
    ...
    {
        $siteName = rescue(fn () => app(SiteSettings::class)->site_name, '3Flo LicenseHub') ?: '3Flo LicenseHub';

        $siteLogo = asset('logo.png');
        if ($logo = rescue(fn () => app(SiteSettings::class)->site_logo, null)) {
            $siteLogo = asset('storage/'.$logo);
        }

        $siteLogoDark = null;
        if ($logoDark = rescue(fn () => app(SiteSettings::class)->site_logo_dark, null)) {
            $siteLogoDark = asset('storage/'.$logoDark);
        }

        $siteFavicon = asset('logo.png');
        if ($favicon = rescue(fn () => app(SiteSettings::class)->site_favicon, null)) {
    ...
    public function boot(): void
    {
        ...
        FilamentView::registerRenderHook(
            ...
            function (): string {
                $siteName = rescue(fn () => app(SiteSettings::class)->site_name, '3Flo LicenseHub');

                $ogImage = asset('og-image.png');
                if ($og = rescue(fn () => app(SiteSettings::class)->og_image_url, null)) {
